...
export default function AdminOrgStructure() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('/organization/getByCategory?category=TASHKILIY_TUZILMA', {
                headers: {
        ...
        <>
            <div className="pt-[75px] pb-[50px]">
                <div className="Admin__header__wrapper flex justify-between items-center mb-4">
                    <h1 className="text-2xl font-bold">Tashkiliy tuzilmasi
                    </h1>
                    {data?.length > 0 ? ('') : (
                        <button
            ...
            <OrgStructureCreate refresh={fetchData} isOpen={createModal} onClose={() => setCreateModal(false)} />
            <OrgStructureEdit refresh={fetchData} isOpen={editModal} onClose={() => setEditModal(false)} data={editData} />
        </>
    );
}